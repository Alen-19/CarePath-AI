  ...
  rejectDoctor(doctor: DoctorVerificationItem, event?: Event): void {
    ...
    this.adminService.rejectDoctor(doctor._id).subscribe({
      next: (res) => {
        this.actionInProgressId = null;
        if (res.success) {
          this.showToast(`Doctor Dr. ${doctor.firstName} ${doctor.lastName}'s verification set to pending/rejected.`, 'error');
          this.loadStats();
          this.loadDoctors();
          if (this.selectedDoctorForModal?._id === doctor._id) {
            this.selectedDoctorForModal = res.doctor;
      ...
      error: (err) => {
        ...
        this.showToast(err.error?.message || 'Failed to reject doctor.', 'error');
      }
    });
  }
